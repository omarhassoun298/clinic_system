...
class ModelAccountCustomer extends Model {
	...
	public function editCustomerById($customer_id, $data) {
		...
		$this->db->query("UPDATE " . DB_PREFIX . "customer SET firstname = '" . $this->db->escape($data['firstname']) . "', lastname = '" . $this->db->escape($data['lastname']) . "', email = '" . $this->db->escape($data['email']) . "', telephone = '" . $this->db->escape(str_replace('+', '', $data['telephone'])) . "', fax = '" . $this->db->escape($data['fax']) . "' WHERE customer_id = '" . (int)$customer_id . "'");
	}
	...
	public function editCustomer($customer_id, $data) {
		$this->db->query("UPDATE " . DB_PREFIX . "customer SET firstname = '" . $this->db->escape($data['firstname']) . "', lastname = '" . $this->db->escape($data['lastname']) . "', email = '" . $this->db->escape($data['email']) . "', telephone = '" . $this->db->escape(str_replace('+', '', $data['telephone'])) . "', dob = '" . $this->db->escape($data['dob']) . "', nationality = '" . $this->db->escape($data['nationality']) . "', occupation = '" . $this->db->escape($data['occupation']) . "', company = '" . $this->db->escape($data['company']) . "', annual_income = '" . $this->db->escape($data['annual_income']) . "', employment_status = '" . $this->db->escape($data['employment_status']) . "', terms_and_conditions = '" . (int)$data['terms_and_conditions'] . "', newsletter = '" . (int)$data['newsletter'] . "', completed = '1', image = '" . $this->db->escape($data['image']) . "' WHERE customer_id = '" . (int)$customer_id . "'");

		if(!empty($data['address'])) {
			$this->db->query("DELETE FROM " . DB_PREFIX . "address WHERE customer_id = '" . (int)$this->customer->getId() . "'");
			$this->db->query("INSERT INTO " . DB_PREFIX . "address SET customer_id = '" . (int)$customer_id . "', firstname = '" . $this->db->escape($data['firstname']) . "', lastname = '" . $this->db->escape($data['lastname']) . "', company = '', address_1 = '" . $this->db->escape($data['address']['address_1']) . "', address_2 = '', postcode = '" . $this->db->escape($data['address']['postcode']) . "', city = '" . $this->db->escape($data['address']['city']) . "', zone_id = '" . (int)$data['address']['zone_id'] . "', country_id = '" . (int)$data['address']['country_id'] . "'");
	...
	public function getTotalCustomersByTelephone($telephone) {
		$query = $this->db->query("SELECT COUNT(*) AS total FROM " . DB_PREFIX . "customer WHERE telephone = '" . $this->db->escape(utf8_strtolower(str_replace('+', '', $telephone))) . "'");
